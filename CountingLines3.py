...
This program monitors the number of lines in the output files of YT-Crawler to give a feedback on the status of the crawling
...
while True:
    ...
    epoch_now = time.time() #getting the time stamp
    frmt_date = dt.datetime.utcfromtimestamp(epoch_now) #converting to a datetime object
    frmt_date=frmt_date+dt.timedelta(hours=2) #adding two hours to adjust to the correct time zone West Europe
    frmt_date = frmt_date.strftime("%Y/%m/%d %H:%M") #printing the string
    ...
    print (
            'line in file = ',counts[j],'time = ' ,frmt_date, ' out of total =', total,'\n',  
    ...
    #Checking if the crawler is blocked
    if (j % 12 == 11):
        total_prev=[]
        for i in range(1,6):
            total_prev.append(counts[j-i]+counts1[j-i]+counts2[j-i]+counts3[j-i]+counts4[j-i]+counts5[j-i]+counts6[j-i])
        if (total_prev[4]==total):
            print("Warning: Crawler blocked since 5*300s \n Stopping to avoid useless plots")
            break
